...
export default function TimetableView() {
  ...
  const getEntityName = (id: string, type: 'class' | 'faculty' | 'subject') => {
    switch (type) {
      case 'class': return classes.find(c => c.id === id)?.name || 'N/A';
  ...
  if (timetable.length === 0) {
    return <div>No timetable generated yet. Please generate one first.</div>;
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <div className="flex space-x-4">
              <label className="flex items-center"><input type="radio" value="class" checked={viewMode === 'class'} onChange={() => { setViewMode('class'); setSelectedEntityId(''); }} className="mr-2" /> By Class</label>
              <label className="flex items-center"><input type="radio" value="faculty" checked={viewMode === 'faculty'} onChange={() => { setViewMode('faculty'); setSelectedEntityId(''); }} className="mr-2" /> By Faculty</label>
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select {viewMode === 'class' ? 'Class' : 'Faculty'}</label>
            <select value={selectedEntityId} onChange={e => setSelectedEntityId(e.target.value)} className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700">
        ...
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
              ...
              {sortedPeriods.map(p => (
                <th key={p.id} scope="col" className="px-6 py-3 text-center">
                  <div>{p.name}</div>
          ...
          <tbody>
            {weekDays.map(day => (
              <tr key={day} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">{day}</th>
                {sortedPeriods.map(p => {
                  if (p.isBreak) {
                    return <td key={p.id} className="px-6 py-4 text-center bg-gray-50 dark:bg-gray-700 font-semibold">{p.name}</td>;
                  }
